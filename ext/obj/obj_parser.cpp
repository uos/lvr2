...
int obj_parse_mtl_file(char *filename, list *material_list)
{
  ...
  if(mtl_file_stream == 0)
    {
      cerr << "[obj_parser] Error reading file '" << filename << "'" << endl;
      return 0;
    }
  ...
  while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
      ...
      else if( strequal(current_token, "map_Ka") && material_open)
	{
	  ...
	  // add path to .ppm file
	  addPathTo(current_mtl->texture_filename, filename, current_mtl->texture_filename);
	  // cout << "[obj_parser] Reading texture information from '" << current_mtl->texture_filename << "'" << endl;
	}
      // texture map
	...
	{
	  cerr << "[obj_parser] Unknown command '" << current_token << "' in material file '" << filename << "' at line " << line_number << " :" << endl << "\t" << current_line << endl;
	  //return 0;
	}
    }
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, char *filename)
{
  ...
  while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
      ...
      else if( strequal(current_token, "mtllib") ) // mtllib
	{
	  ...
	  addPathTo(growable_data->material_filename, filename, growable_data->material_filename);
	  cout << "[obj_parser] Reading material information from '" << growable_data->material_filename << "'" << endl;
	...
	{
	  cerr << "[obj_parser] Unknown command '" << current_token << "' in scene code at line " << line_number << " :" << endl << "\t" << current_line << endl;
	}
    }
