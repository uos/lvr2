...
kfusion::KinFu::KinFu(KinFuParams* params) : frame_counter_(0), params_(params), has_shifted_(false), perform_last_scan_(false), perform_shift_(false)
                                                   , cyclical_(params), checkForShift_(true)

{
    CV_Assert(params->volume_dims[0] % 32 == 0);

    volume_ = cv::Ptr<cuda::TsdfVolume>(new cuda::TsdfVolume(params->volume_dims));

    volume_->setTruncDist(params->tsdf_trunc_dist);
    volume_->setMaxWeight(params->tsdf_max_weight);
    volume_->setSize(params->volume_size);
    volume_->setPose(params->volume_pose);
    volume_->setRaycastStepFactor(params->raycast_step_factor);
    volume_->setGradientDeltaFactor(params->gradient_delta_factor);

    // initialize cyclical buffer
    cyclical_.initBuffer(volume_);

    icp_ = cv::Ptr<cuda::ProjectiveICP>(new cuda::ProjectiveICP());
    icp_->setDistThreshold(params->icp_dist_thres);
    icp_->setAngleThreshold(params->icp_angle_thres);
    icp_->setIterationsNum(params->icp_iter_num);
    allocate_buffers();
    reset();
}
...
void kfusion::KinFu::reset(Affine3f initialPose)
{
    if (frame_counter_)
        cout << "Reset" << endl;
    //initialPose.translate(Vec3f(0, 0, params->distance_camera_target));
    frame_counter_ = 0;
    poses_.clear();
