...
results in a `lvr2-X-Linux.tar.gz` which can be extracted afterwards to any root dir:

```bash
mkdir ~/software
tar -C ~/software -xzf lvr2-X-Linux.tar.gz
```

you can include this by adding updating the paths. For example in the `.bashrc` file:

```bash
export LVR2_INSTALL_DIR=~/software/lvr2-25.1.0-Linux
