	...
	: BaseOption(argc, argv)
{
	...
	m_descr.add_options()
		        ...
		        ("decomposition,d", value<string>(&m_pcm)->default_value("MC"), "Defines the type of decomposition that is used for the voxels (Standard Marching Cubes (MC), Planar Marching Cubes (PMC), Standard Marching Cubes with sharp feature detection (SF) or Tetraeder (MT) decomposition. Choose from {MC, PMC, MT, SF}")
		        ...
		        ("threads", value<int>(&m_numThreads)->default_value( 1 ), "Number of threads")
