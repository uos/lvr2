...
void LBVHIndex::build(float* points, size_t num_points)
{
    ...
    // initialize AABBs
    AABB* aabbs = (AABB*) malloc(sizeof(AABB) * num_points);
    ...
    // Get the extent
    AABB* extent = getExtent(points, m_num_objects);
    ...
    //         (aabbs, extent, d_morton_codes, m_num_objects);
...
__device__ __host__ 
AABB* LBVHIndex::getExtent(float* points, size_t num_points)
{
    float min_x = INT_MAX;
    float min_y = INT_MAX;
    float min_z = INT_MAX;
