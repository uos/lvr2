...
void ClSurface::calculateNormals()
{
    ...
    // {
    //     std::cout << "Point" << i << ": " << this->V.elements[i*3] << "|" << this->V.elements[i*3+1] << "|" << this->V.elements[i*3+2] << std::endl;
    // }
...
void ClSurface::initCl()
{
    ...
    // int gpu = 0;
    // m_ret = clGetDeviceIDs(NULL, gpu ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 1, &this->m_device_id, NULL);
    // if (m_ret != CL_SUCCESS)
    // {
    //     std::cerr << getErrorString(m_ret) << std::endl;
    // }
