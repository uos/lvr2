...
{
	int index_map[m_vertices.size()*2];
	...
	for(size_t i = 0; vertices_iter != vertices_end; ++i, ++vertices_iter)
	{
		...
		index_map[vertices_iter->first] = i;
	}
	...
	{
		this->m_indexBuffer[3 * i]      = index_map[(*face_iter)->m_index[0]];
		this->m_indexBuffer[3 * i + 1]  = index_map[(*face_iter)->m_index[1]];
		this->m_indexBuffer[3 * i + 2]  = index_map[(*face_iter)->m_index[2]];
