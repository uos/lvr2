...
namespace lvr2
{
    ...
    SoilAssistField::SoilAssistField(std::string name, PolygonPtr boundary, std::vector<SoilAssistSubFieldPtr>& subfields) : m_name(name), m_boundary(boundary), m_subfields(subfields)
    {
    }
    ...
    PolygonPtr SoilAssistField::getBoundary()
    {
        return m_boundary;
    }
    ...
    void SoilAssistField::setBoundary(PolygonPtr boundary)
    {
        m_boundary = boundary;
    }
    ...
    void SoilAssistField::fromH5File(std::string path)
    {
        ...
        this->setBoundary(PolygonPtr(new Polygon(hborder)));
        if(hdf5_file.exist("/field/subfields"))
        {
            ...
            for(auto subFieldName : subFieldNames)
            {
                ...
                if(hdf5_file.exist(boundary_path))
                {
                    ...
                    subfield->setBoundary(PolygonPtr(new Polygon(hborder)));
                }
                ...
                if(hdf5_file.exist("/field/subfields/" + subFieldName + "/reference_lines"))
                {
                    ...
                    for(auto && refLineName : refLineNames)
                    {
                        ...
                        if(hdf5_file.exist(ref_path))
                        {
                            ...
                            subfield->addReferenceLine(PolygonPtr(new Polygon(hpoints)));
                        }
                    }
                }
                // headlands:
                if(hdf5_file.exist("/field/subfields/" + subFieldName + "/headlands"))
                {
                    ...
                    for(auto && headlandName : headlandNames)
                    {
                        ...
                        {
                            std::vector<std::vector<float> > hpoints = H5Easy::load<std::vector<std::vector<float> > >(hdf5_file,ref_path );
                            subfield->addHeadland(PolygonPtr(new Polygon(hpoints)));
                        }
                    }
                }
